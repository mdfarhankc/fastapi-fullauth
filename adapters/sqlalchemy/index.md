...
`UserMixin` provides `id`, `email`, `hashed_password` (nullable; `NULL` for OAuth-only users), `is_active`, `is_verified`, `is_superuser`, `created_at`. For integer or string user keys, see [Choosing a primary key type](https://mdfarhankc.github.io/fastapi-fullauth/adapters/#choosing-a-primary-key-type).
