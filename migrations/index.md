...
1. Pass the model to the adapter:
...
[Session management](https://mdfarhankc.github.io/fastapi-fullauth/auth/sessions/index.md) records where each sign-in came from, adding two **nullable** columns to `fullauth_refresh_tokens`: `user_agent` and `ip_address`. Both are nullable, so existing rows stay `NULL` and the change is additive — no backfill, no new table (the `sessions` router reads the existing refresh-token rows grouped by `family_id`).
