...
`DELETE /auth/passkeys/{id}` refuses with `400` when that passkey is the account's only way to sign in, the same rule [unlinking a provider](https://mdfarhankc.github.io/fastapi-fullauth/oauth/#unlinking-providers) follows. Set a password or register a second passkey first.
...
1. The server only accepts the new count if it's strictly greater than the stored value (atomic CAS via `update_passkey_sign_count()`).
1. If the CAS fails and the reported count is non-zero, authentication is rejected. This means another device already used a higher count, indicating a clone.
