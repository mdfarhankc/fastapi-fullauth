...
1. **`RateLimitMiddleware`**: a generic per-IP request limiter. Not wired automatically; add it yourself with `app.add_middleware(...)`.
1. **Account lockout**: locks accounts after too many failed login attempts. Different from rate limiting: lockout is per-account (by email), rate limiting is per-IP.
