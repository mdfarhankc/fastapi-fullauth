...
`X-XSS-Protection` is `0` on purpose
...
1. On state-changing requests (`POST`, `PUT`, `PATCH`, `DELETE`), the client must send the cookie value in the `X-CSRF-Token` header
1. The middleware verifies the HMAC-SHA256 signature on the cookie and compares the cookie value against the header
...
See [Rate Limiting](https://mdfarhankc.github.io/fastapi-fullauth/security/rate-limiting/index.md) for the full guide on both auth rate limits and the global middleware.
