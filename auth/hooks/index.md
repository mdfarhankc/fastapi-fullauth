...
Mistakes surface at registration
...
Registering does not send a verification email by itself: `/register` only fires `after_register`, and `send_verification_email` is emitted by `POST /verify-email/request`, which requires the user to be signed in already. To mail a link at signup, mint the token in the hook:
...
This hook runs after the response is sent, so the time your mail provider takes cannot reveal whether the address was already registered.
