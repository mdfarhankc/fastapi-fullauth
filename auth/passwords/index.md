...
1. Validate the new password against the password validator
1. Hash and store the new password
1. Revoke all refresh tokens (forces re-login on all devices)
1. Fire the `after_password_change` hook
...
1. Generate a purpose-scoped JWT with `purpose=password_reset` and a short TTL (default 15 minutes)
1. Fire the `send_password_reset_email` hook with the email and token
1. Return 202 regardless of whether the email exists (prevents enumeration)
...
1. Hash and store the new password
1. Blacklist the reset token (prevents reuse)
1. Revoke all refresh tokens (forces re-login on all devices)
1. Fire the `after_password_reset` hook
