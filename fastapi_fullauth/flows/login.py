...
from fastapi_fullauth.core.crypto import ahash_password, averify_password, password_needs_rehash
...
async def _get_dummy_hash(algorithm: Literal["argon2id", "bcrypt"] = "argon2id") -> str:
    """Lazily build a valid hash so timing-defense verifies take real time.

    Cached per algorithm: the dummy verify must use the same hasher the real
    users do, otherwise a bcrypt deployment still leaks user existence through
    the argon2-vs-bcrypt timing difference on the unknown-user path.
    """
    if algorithm not in _DUMMY_HASHES:
        _DUMMY_HASHES[algorithm] = await ahash_password(
            "fastapi-fullauth-timing-defense", algorithm=algorithm
        )
    return _DUMMY_HASHES[algorithm]
...
async def login(
    ...
    if password_needs_rehash(hashed, algorithm=hash_algorithm):
        try:
            await adapter.set_password(
                user.id, await ahash_password(password, algorithm=hash_algorithm)
            )
        except Exception:
    ...
    if extra_claims_provider is not None:
        extra_claims = await extra_claims_provider(user)
