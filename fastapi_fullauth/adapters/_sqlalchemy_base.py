"""Shared async implementation for the SQLAlchemy and SQLModel adapters.

SQLModel is built on SQLAlchemy, so both adapters run the exact same Core/ORM
queries. The only real differences are the model mixin types accepted by each
public constructor (separate class hierarchies) and the adapter name shown in
error messages. Everything else lives here once.

This module is internal: import ``SQLAlchemyAdapter`` or ``SQLModelAdapter``
from their respective modules instead.
"""
...
class _BaseSQLAlchemyAdapter(
    ...
    Subclasses provide a precisely typed ``__init__`` for their ORM and forward
    to ``_configure``; they set ``_adapter_name`` for error messages.
    """

    _adapter_name = "the adapter"

    # Set on the bound clone returned by transaction(); None on the startup
    # singleton so normal calls open their own session.
    _tx_session: AsyncSession | None = None
    ...
    def _configure(
        ...
        *,
        ...
        role_model: Any = None,
        user_role_model: Any = None,
        permission_model: Any = None,
        role_permission_model: Any = None,
        oauth_account_model: Any = None,
        passkey_model: Any = None,
        user_schema: type[UserSchemaType],
        create_user_schema: type[CreateUserSchemaType],
    ) -> None:
    ...
    @asynccontextmanager
    async def transaction(self) -> AsyncIterator["_BaseSQLAlchemyAdapter[Any, Any]"]:
        """Run several adapter calls inside a single transaction.

        Yields an adapter bound to one session. Calls on the yielded object
        commit together when the block exits, or roll back entirely if it
        raises. Calls on the original adapter are unaffected.

            async with adapter.transaction() as tx:
                user = await tx.create_user(...)
                await tx.assign_role(user.id, "admin")

        Works out of the box on PostgreSQL and MySQL. On SQLite the pysqlite/
        aiosqlite driver emits BEGIN lazily, which breaks SAVEPOINT and rollback;
        configure the engine with SQLAlchemy's documented BEGIN-emulation recipe
        for correct behavior there.
        """
        if self._tx_session is not None:
            raise RuntimeError("transaction() cannot be nested.")
        async with self._session_maker() as session, session.begin():
            bound = copy(self)
            bound._tx_session = session
            yield bound

    @asynccontextmanager
    async def _begin(self) -> AsyncIterator[AsyncSession]:
        """Reuse the active transaction's session, or open a short-lived one."""
        if self._tx_session is not None:
    ...
    async def _commit(self, session: AsyncSession) -> None:
        """Persist the work, or defer to transaction() when one is active.

        Inside a transaction a flush still emits the SQL, so IntegrityError and
        UPDATE rowcount behave exactly as a commit would; the real commit runs
    ...
    async def _insert(self, session: AsyncSession, obj: Any) -> None:
        ...
        Inside a transaction the add+flush run in a SAVEPOINT so a conflict rolls
        back only this object; standalone, a failed flush rolls back the session.
        """
        if self._tx_session is session:
    ...
    async def create_user(
        ...
        async with self._begin() as session:
            ...
            try:
                await self._insert(session, user)
