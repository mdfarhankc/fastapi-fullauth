...
List settings take a comma-separated string

`ORIGINS`, `TRUSTED_PROXY_HEADERS`, `PASSKEY_ORIGINS`, and `ROUTER_TAGS` accept a plain comma-separated value from the environment, so you don't have to write JSON:
...
Prints a random key suitable for `FULLAUTH_SECRET_KEY`. The `fullauth` command ships with the package.
...
Loads `FULLAUTH_*` from the environment and `.env`, prints the resolved settings, and reports the warnings the app would emit at startup (a generated secret key, in-memory backends under multi-worker deployments, and so on). Because `BACKEND` and `PASSKEY_ENABLED` are inferred from `REDIS_URL` and `PASSKEY_RP_ID`, this is the quickest way to confirm what the library actually resolved before you boot. It exits non-zero when the config fails to construct, so it works as a CI pre-flight check.
...
1. Process environment: `os.environ["FULLAUTH_SECRET_KEY"]`
1. `.env` file
1. Field defaults
...
Per-route auth rate limits are baked into the routers. Global request-rate middleware (`RateLimitMiddleware`) is opt-in; import it from `fastapi_fullauth.middleware` and call `app.add_middleware(...)` yourself.
...
| `TRUSTED_PROXY_COUNT`            | `int`                 | `1`        | Number of trusted proxy hops in front of the app. The client IP is the entry this many positions from the right of the forwarded chain. Must be `>= 1`. Set it too high and an attacker can spoof their IP; see [Rate Limiting](https://mdfarhankc.github.io/fastapi-fullauth/security/rate-limiting/#proxy-support). |
...
Override individual routes without touching the others. In Python, pass an `AuthRateLimits` (importable from `fastapi_fullauth`); unset fields keep their defaults:
...
From the environment, set the field as a JSON object (only the keys you want to change):
...
`init_app()` does not wire middleware automatically. Import what you need from `fastapi_fullauth.middleware` (`SecurityHeadersMiddleware`, `CSRFMiddleware`, `RateLimitMiddleware`) and `app.add_middleware(...)` it yourself.

`CSRFMiddleware` takes its signing key directly; there's no config field for it. Pass `config.SECRET_KEY` (≥ 32 chars), or a dedicated key if you want to rotate it independently:
...
Cookie attributes live on the `CookieBackend` constructor (the cookie backend is opt-in), not on `FullAuthConfig`. Pass them when you wire it up:
