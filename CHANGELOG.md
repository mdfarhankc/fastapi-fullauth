...
- **Link an OAuth provider while signed in.** Until now a provider could only be attached as a side effect of signing in, and only when the provider reported the email as verified and it already matched an account, which left out a work account under a different address, a provider that does not verify emails, and one that returns no email at all. `GET /oauth/{provider}/link/authorize` and `POST /oauth/{provider}/link/callback` (both authenticated, same `binding` contract as sign-in) now attach the identity to the caller's account and return it; no tokens are issued, since linking is not a sign-in. The provider email is stored for display only, so it does not have to match the account and does not verify it. Two conflicts answer `409`: the identity already belongs to another user (never moved, that would be account takeover), or the user already linked a different account from the same provider (unlinking is keyed by provider alone and could not tell them apart). Linking an account the user already has refreshes the stored provider tokens. New flow functions `link_oauth_account()`, `oauth_link_callback()`, and `build_link_authorization_url()`; new exceptions `OAuthAccountAlreadyLinkedError` and `OAuthProviderAlreadyLinkedError`. This closes the last `get_oauth_associate_router` gap in the [fastapi-users migration guide](https://mdfarhankc.github.io/fastapi-fullauth/migrating-from-fastapi-users/).
- **Refresh-token pruning.** `adapter.prune_expired_refresh_tokens(before=None)` deletes rows whose expiry has passed and returns how many went. Rotation writes a row per refresh, so the table only ever grew, and `GET /sessions` reads every row a user has ever had. Only expired rows are removed: a revoked but unexpired row is what a replayed token is matched against, and deleting it early would turn a stolen-token replay into an ordinary rejection and leave the family alive. Call it from your own scheduler; nothing runs it for you. Custom adapters inherit a no-op, so nothing breaks.
- **`PASSWORD_MAX_LENGTH` (default 4096).** Without an upper bound, a large request body became hashing work for the server. bcrypt already refused over 72 bytes; argon2id would hash whatever arrived.
- **`REAUTH_MAX_AGE_SECONDS` (default 300) and an `auth_time` claim.** Tokens now record when the credentials behind the session were actually checked, stamped at login, OAuth callback and passkey sign-in, and carried unchanged through refresh rotation. New flow helpers `verify_recent_auth()` and `remaining_login_methods()`.
- **`after_oauth_link` and `after_oauth_unlink` hooks**, both `async def(user, provider)`. Attaching or removing a sign-in method is an event applications need to audit-log, and unlinking had no hook at all.

### Changed

- **The Redis lockout manager fails open instead of raising.** It had no error handling at all, so with `LOCKOUT_BACKEND=redis` a Redis outage turned every login into a 500. Attempts are now neither counted nor blocked while Redis is unreachable, and the error is logged; brute-force protection is lost for the duration. The rate limiter already made this trade; the token blacklist still fails closed.
- **`refresh_tokens.user_id`, `refresh_tokens.expires_at` and `oauth_accounts.user_id` are indexed** in the SQLModel and SQLAlchemy mixins. Every session listing, revoke-all, and prune filters on them, and a foreign key does not imply an index on PostgreSQL. The Beanie documents already indexed them. **Migration:** existing deployments need to add these indexes; new tables get them automatically. For Alembic, autogenerate picks them up, or add them by hand:

    ```sql
    CREATE INDEX ix_fullauth_refresh_tokens_user_id ON fullauth_refresh_tokens (user_id);
    CREATE INDEX ix_fullauth_refresh_tokens_expires_at ON fullauth_refresh_tokens (expires_at);
    CREATE INDEX ix_fullauth_oauth_accounts_user_id ON fullauth_oauth_accounts (user_id);
    ```

- **`POST /verify-email/request` no longer mails an already-verified account.** `create_email_verification_token()` returns `None` for a verified user; the route's response is unchanged, so this leaks nothing.
- **Redis clients are shared per event loop, not process-wide.** A redis-py asyncio client binds its connections to the loop that first used them, so one client handed to a second loop fails at runtime. Acquiring outside a running loop, which is the usual case since `FullAuth` is built at import time, still shares one client per URL.

### Security

- **Deleting an account and setting a first password now require recent authentication.** A valid access token said a session existed, not that whoever held it had proved anything, so a token lifted from a logged-in machine could delete the account outright, or give an OAuth-only account a password and a second way in. Both now need either `current_password` or a session whose credentials were checked within `REAUTH_MAX_AGE_SECONDS`, and answer `403` otherwise. The age comes from `auth_time`, which rotation preserves, so refreshing a stolen token does not renew it. **Breaking:** `DELETE /me` and first-password sets from sessions older than five minutes now fail; clients should prompt for the password and send it in the body, or re-authenticate. Tokens issued before this release carry no `auth_time`, so the session counts as last authenticated when its current token was issued: an access token minted minutes ago still passes, and rotating an old refresh token carries that token's own issue time forward rather than resetting the clock.
- **A password-reset link is invalidated by a password change.** The link stayed usable for its whole TTL even after the password had been changed by another reset link or by `/change-password`, so an older mail in an inbox could undo a deliberate change. The token now carries a digest of the password hash it was issued against, and a reset whose digest no longer matches is rejected and burned. No new storage. **Migration:** reset links issued before the upgrade stop working, up to `PASSWORD_RESET_EXPIRE_MINUTES` (15 by default) after deploy.
- **Removing a sign-in method can no longer lock an account out.** `DELETE /oauth/accounts/{provider}` counted a password and other providers but not passkeys, so it told a passkey user to "set a password first"; `DELETE /passkeys/{id}` had no such check at all, so a passkey-only account could delete its last credential and never get back in. Both routes now ask one shared question, `remaining_login_methods()`, which counts a stored password, every linked provider, and every registered passkey, and the `400` names only the alternatives the deployment actually offers.
- **A link state is bound to the user it was issued for.** The state for a link flow carries that user's id and the callback rejects it unless it matches the authenticated caller, so an attacker who finishes their own provider flow cannot have a victim submit the result and end up attached to the victim's account. The id travels in a claim rather than the token subject, since the state passes through the address bar and the provider and has to stay useless as a session token. Sign-in states and link states carry different purposes and cannot be swapped. `generate_oauth_state()` and `build_authorization_url()` gained keyword-only `purpose` and `claims` parameters, both defaulted, so existing calls are unaffected.

### Fixed

- **The admin permission routes answer 501 instead of 500 without permission models.** The admin router mounts as soon as the adapter supports roles, so `/admin/assign-permission`, `/admin/remove-permission` and `/admin/role-permissions/{role}` existed against adapters configured for roles alone, where the adapter's `RuntimeError` escaped as a 500. Role management on such an adapter is unaffected.
- **The Agent Skill listed eight admin routes that do not exist.** `references/rbac.md` and the route table in `references/api-reference.md` documented user CRUD (`/admin/users`, `/admin/users/{id}/roles`, `/admin/roles/{role}/permissions`) that the library has never shipped, and said admin routes accept `require_role("admin")` when they require `is_superuser`. Both now describe the five routes that exist, and say plainly that there is no user CRUD.
- **The in-memory passkey challenge store never evicted anything.** Challenges were removed only by completing the flow, so every abandoned one stayed for the life of the process, and `POST /passkeys/authenticate/begin` needs no account. It now sweeps expired entries on write, like the blacklist, lockout and rate-limit stores.
- **`fastapi_fullauth.routers` exports every router factory.** It listed four of the seven, so `create_oauth_router`, `create_passkey_router`, and `create_sessions_router` had to be imported from their modules even though the package documented the shorter path.
- **The OAuth router is built once, not twice.** It was the only router property without a cache, and the router builder reads it twice (once to test it, once to mount it), so every `init_app()` built two full routers with their dynamically created response models and discarded one.
- **Signing in no longer attaches a second identity from the same provider.** If two accounts at one provider verify the same email address, auto-link-by-email attached both to the same user. `DELETE /oauth/accounts/{provider}` resolves by provider alone, so unlinking deleted whichever row came first and left the other one attached, with no way to reach it: the user unlinked the provider and stayed linked to it. The second sign-in is now refused, like the link route refuses it, and answers the same generic `400 OAuth authentication failed` as every other sign-in failure. Reachable with custom or self-hosted providers; the major providers do not verify one address on two accounts.
- **An OAuth error that is not an `OAuthProviderError` no longer returns a 500.** Both OAuth callbacks caught `OAuthProviderError`, so any other `OAuthError` subclass escaped as an unhandled exception. They now catch `OAuthError`.
...
- **Corrected the guidance given when auto-link-by-email is refused.** When a provider returns an unverified email that already belongs to an account, the flow refused the link (correctly) but told the user to "link your OAuth account from account settings", which no endpoint supports. It now names the path that the link routes above make real: sign in with a password, then link the provider. The message is internal to `OAuthProviderError`: the router still answers every OAuth failure with a generic `400 OAuth authentication failed`, so nothing about the response changes. The Agent Skill's OAuth reference described the same impossible flow, claiming the callback links through the caller's session; it now documents what the callback actually does.

## 0.16.0

### Added

- **Pluggable user id types.** The user primary key no longer has to be a UUID: `UserSchema` is now generic over its key type, so `class MyUser(UserSchema[int])` gives you integer or sequence keys and `UserSchema[str]` gives you string keys. `class MyUser(UserSchema)` still means UUID, so existing applications need no edits at all. Adapters gain `parse_user_id()`, which converts a token subject back to the declared key type using the user schema you already pass in; every flow and the admin role endpoints (whose `user_id` body field now takes the declared type, also in OpenAPI) use it. The bundled model mixins keep UUID keys, so a non-UUID app overrides `id` on its user model and `user_id` on each related model (Tortoise needs only the user `id`); see [Choosing a primary key type](https://mdfarhankc.github.io/fastapi-fullauth/adapters/#choosing-a-primary-key-type). The passkey table keeps its UUID primary key, since the library generates passkey ids. Integer keys are enumerable in a way UUIDv7 keys are not, so choose deliberately if you expose ids.
- **Fail-fast guard for mismatched key types.** Declaring `UserSchema[int]` over a UUID table used to parse fine, miss every lookup, and return 401 on every request with nothing to point at the cause. Adapters now compare the schema's `id` type against the user model's primary key, and against the `user_id` of every related model, at construction and raise a `ValueError` naming the mismatch. The related-model check catches a forgotten mixin override that SQLite would silently accept and PostgreSQL would reject on insert. Custom adapters opt in by overriding `model_user_id_type()` and `related_user_id_types()`; storage whose key type cannot be read reliably skips the check rather than guessing.
- **Security policy and threat model.** `SECURITY.md` explains how to report a vulnerability privately, and a new [threat model](https://mdfarhankc.github.io/fastapi-fullauth/security/threat-model/) page documents what the library defends against, its trust assumptions, its deliberate failure modes, and what stays the application's responsibility.
...
- **`llms.txt` and `llms-full.txt` are generated from the documentation at build time.** They were maintained by hand and had drifted from the docs. The URLs are unchanged, and every page is now also published as Markdown (for example `/adapters/index.md`), which the `llms.txt` index links to.
...
- **OAuth login is protected against login CSRF.** The OAuth `state` was signed but not bound to the browser, so an attacker could start a login with their own provider account and make a victim's browser finish it, signing the victim into the attacker's account. The server-derived PKCE verifier did not prevent this. As RFC 9700 requires, the state is now bound to the client: `GET /oauth/{provider}/authorize` returns a random `binding` next to `authorization_url`, the state carries only its SHA-256 digest, and `POST /oauth/{provider}/callback` requires the `binding` back (422 when missing, 400 when it does not match). New helper `generate_oauth_binding()`; `build_authorization_url`, `generate_oauth_state`, `verify_oauth_state`, `exchange_oauth_code`, and `oauth_callback` take a required keyword-only `binding`. **Breaking, migration:** store `binding` from the authorize response in `sessionStorage` and include it in the callback body, `{"code": ..., "state": ..., "binding": ...}`. See the [frontend guide](https://mdfarhankc.github.io/fastapi-fullauth/frontend-integration/).
- **Ending a session invalidates its access tokens immediately.** Logout, revoking a device, revoke-others, a password change, and a password reset only revoked refresh tokens, so access tokens already issued to those sessions kept working until they expired (30 minutes by default). Each session's `family_id` is now blacklisted for the access-token lifetime, and tokens of a blacklisted family are rejected. `TokenBlacklist` gains a non-abstract `is_any_blacklisted()` (one `EXISTS` call on Redis, still fail-closed), so custom backends keep working; `TokenEngine.revoke_family()` is new. **Behaviour change:** `/auth/change-password` now ends every session at once, including the caller's, so clients should expect a 401 and re-authenticate. Instant revocation across workers needs the Redis blacklist.
- **Refresh-token reuse detection now works with the blacklist enabled.** Rotation blacklisted the old refresh token's `jti`, so replaying it was rejected at decode and never reached the compare-and-swap that revokes the family. If an attacker refreshed with a stolen token first, the legitimate user's replay was merely refused and the attacker kept the session. The rotated token is no longer blacklisted, so a replay now revokes the whole family, including access tokens it issued. This also stops writing a 30-day blacklist entry on every refresh.
- **Account lockout can no longer be bypassed by changing email case.** Lockout counted failures under the identifier exactly as typed while lookups normalise email, so `Victim@x.com` and `VICTIM@x.com` had separate counters and guessing could continue past `MAX_LOGIN_ATTEMPTS`. Lockout now keys on the normalised email.
- **`PATCH /me` enforces the user schema.** The update body is rebuilt with every field optional, which dropped field constraints, custom validators, and nullability, so a client could store values the schema forbids (and the request then failed with a 500 on read-back). Updates are now merged over the current user and validated against the schema before anything is written; violations return FastAPI's standard 422. `validate_profile_updates` gains an optional `current` argument.
- **Registration and password-reset response times no longer reveal registered emails.** With `PREVENT_REGISTRATION_ENUMERATION` on, a duplicate email returned immediately while a new one paid for a password hash and ran `after_register` inline; reset requests sent the email inline only for real accounts. The duplicate path now performs the same hash, and `after_register`, `send_verification_email`, and `send_password_reset_email` run as background tasks after the response. **Behaviour change:** those three hooks now run after the response is sent.
- **Registration no longer persists privileged fields from the create schema.** Adapters passed every `CreateUserSchema` field except `email` and `password` straight to the user model, so a create schema that exposed `is_superuser`, `is_verified`, `is_active`, `hashed_password`, or `roles` let a registering client grant itself admin or choose its own password hash. New `create_user_extra_fields()` drops `PRIVILEGED_USER_FIELDS` (both exported from `fastapi_fullauth.adapters`); the bundled adapters use it, and custom adapters should too.
- **OAuth refuses deactivated accounts before writing.** A refused sign-in still linked the provider identity to the deactivated account, or refreshed the stored provider tokens of an existing link. The active check now runs first.

### Fixed

- **Logout works when the access token has expired.** Logout required a valid access token, so an idle tab whose access cookie had lapsed got a 401, kept both cookies, and left its session alive. Logout now falls back to the refresh token (cookie or body) through the new `logout_with_refresh_token()` flow, and clears token cookies on every outcome, including a 401.
- **OAuth provider failures return 400 instead of 500.** Timeouts, refused connections, and non-JSON or non-object responses from a provider escaped `StandardOAuthProvider` unhandled. They now raise `OAuthProviderError`. GitHub's emails lookup treats these failures like a non-200 and keeps the profile email unverified instead of failing the login.
- **Verify-email and password-reset confirm return 401 for deleted accounts.** A still-valid token for an account that no longer exists raised `UserNotFoundError` and returned 500.
- **A Redis outage no longer makes `RateLimitMiddleware` return 500.** `RedisRateLimiter.remaining()` and `reset_time()` raised during an outage while `is_allowed()` failed open. They now fail open too.
- **The in-memory blacklist, lockout manager, and rate limiter evict expired entries.** They only pruned the key being looked up, so entries nobody asked about again, including attacker-chosen login identifiers and client IPs, grew memory for the life of the process. Each now sweeps expired entries at most once a minute.
- **A passkey challenge stored with a TTL of 0 now expires immediately on coarse clocks.** The expiry check used a strict comparison, so on platforms whose monotonic clock ticks every ~15 ms (Windows) an immediate lookup could still return the challenge.
- **One unknown passkey transport no longer breaks passkey sign-in.** Registration stored client-supplied transports verbatim, and building authentication options raised on any value WebAuthn does not define. Registration now keeps only known transports, and unknown stored values are ignored.

## 0.15.0

### Added

- **Tortoise ORM adapter.** A new `TortoiseAdapter` (`from fastapi_fullauth.adapters import TortoiseAdapter`) plus a matching set of abstract Tortoise model mixins under `fastapi_fullauth.models.tortoise` (`UserMixin`, `RefreshTokenMixin`, `RoleMixin`, `PermissionMixin`, `OAuthAccountMixin`, `PasskeyMixin`), for apps built on [Tortoise ORM](https://tortoise.github.io/). It implements the full feature set - roles, permissions, OAuth, passkeys, sessions, and atomic refresh-token rotation (via Tortoise's `in_transaction`, with SAVEPOINT isolation for conflict-prone inserts). Roles and permissions use native Tortoise many-to-many relations you declare on your `User` / `Role` models, so the adapter takes no `user_role_model` / `role_permission_model`. Name your concrete models `User` / `Role` / `Permission` and register them under the Tortoise app label `models`. Install with `pip install fastapi-fullauth[tortoise]` (or `[tortoise-standard]` for the adapter plus every optional feature). See the [Tortoise adapter guide](https://mdfarhankc.github.io/fastapi-fullauth/adapters/tortoise/).
- **Beanie (MongoDB) adapter.** A new `BeanieAdapter` (`from fastapi_fullauth.adapters import BeanieAdapter`) plus ready-to-use Beanie document classes under `fastapi_fullauth.models.beanie` (`UserDocument`, `RefreshTokenDocument`, `RoleDocument`, `PermissionDocument`, `OAuthAccountDocument`, `PasskeyDocument`), for apps on [MongoDB](https://www.mongodb.com/) via [Beanie](https://beanie-odm.dev/). It implements the full feature set - roles, permissions, OAuth, passkeys, and sessions. MongoDB has no foreign keys or join tables, so role membership is an embedded array on the user document and a role's permissions are an embedded array on the role document; the adapter takes no `user_role_model` / `role_permission_model`. Refresh-token reuse detection is a single-document atomic compare-and-swap (`find_one_and_update`), so rotation stays correct **without a replica set or multi-document transactions** - `transaction()` is best-effort, and a crash mid-rotation orphans at most one refresh token without ever weakening reuse detection. `delete_user` removes the user's refresh-token / OAuth / passkey documents explicitly, since MongoDB does not cascade. Install with `pip install fastapi-fullauth[beanie]` (or `[beanie-standard]` for the adapter plus every optional feature). See the [Beanie adapter guide](https://mdfarhankc.github.io/fastapi-fullauth/adapters/beanie/).
...
- New [Custom Adapters](https://mdfarhankc.github.io/fastapi-fullauth/adapters/custom/) guide with a complete, runnable in-memory adapter example, the key method contracts, and how to opt into roles, permissions, OAuth, passkeys, and sessions.
- New [Customization](https://mdfarhankc.github.io/fastapi-fullauth/customization/) hub linking every extension point, and a [Recipes](https://mdfarhankc.github.io/fastapi-fullauth/recipes/) page with end-to-end examples (multi-tenant SaaS, username login).
