...
1. If the CAS succeeded (caller won the race), issues a new token pair in the same family
1. If the CAS failed (someone else already revoked it), the entire family is revoked and the request is rejected
...
1. **Token decoding**: `decode_token` verifies the JWT signature using `SECRET_KEY`, checks expiry (with `JWT_LEEWAY_SECONDS` tolerance for clock skew), and queries the blacklist.

1. **Type validation**: the dependency checks that `token.type == "access"` (not a refresh token) and that the token has no `purpose` field (not a verification/reset token).

1. **User lookup**: the user is loaded by `sub` (user ID) from the adapter.

1. **Active check**: if the user is inactive (`is_active=False`), the request gets a 401.

1. **Additional checks** (depending on the dependency used):
...
See [Event Hooks](https://mdfarhankc.github.io/fastapi-fullauth/auth/hooks/index.md) for the full list with callback signatures.
...
See [Custom Token Claims](https://mdfarhankc.github.io/fastapi-fullauth/auth/custom-claims/index.md) for details.
